#!/bin/bash
# Submit singer runs as a SLURM array on extended-40core-shared (Skylake).
# Each array task runs snakemake for one row of the params CSV.
# Usage: bash submit_skylake.sh [start] [end]
#   defaults: 1 700
...
PARTITION="extended-40core-shared"
MAX_TIME="48:00:00"
MEM="2G"
CPUS=1
...
sbatch <<EOF
#!/bin/bash
#SBATCH --job-name=singer_skylake
...
# Per-task working directory keeps .snakemake/ state isolated across concurrent tasks
